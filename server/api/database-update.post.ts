...
export default defineEventHandler(async (event) => {
    ...
    const characters: Character[] = rawCharacters.map(
        (character) => new Character(character.hash, character.full_name, character.file_name, character.timestamp, character.formatted_timestamp, character.image_content, character.id)
    );
